 ...
 */
class TaskUpdateModel(override var context: Context) : TaskBase<Model, Model>(context) {
    override fun build(input: Model?): Observable<Model> {
        return Observable.create { emitter ->
            ...
            var res: Call<Model> = api.loadModels(
                Server.API_MODEL_UPDATE[0],
                Server.API_MODEL_UPDATE[1],
                Server.API_MODEL_UPDATE[2],
                Server.API_MODEL_UPDATE[3],
                input!!.id,
                input.name,
                input.birthDay,
                input.imgUrl
